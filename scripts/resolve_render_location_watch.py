...
def handle_intercept(resolve, quiet):
    ...
    # Erst Resolves Dialog schliessen -> Modal weg -> API wieder ansprechbar.
    time.sleep(0.15)
    send_escape()
    time.sleep(0.20)
    ...
    if not resolve:
        srl.notify("Cannot connect to Resolve.", critical=True)
        return resolve
    ...
    if not project:
        srl.notify("No active project in Resolve.", critical=True)
        return resolve
